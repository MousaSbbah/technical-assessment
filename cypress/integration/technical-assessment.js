...
When('A user filters the results by price range: min-{string}, max-{string}', (minimumPrice, maximumPrice) => {
	propertyFinder.filterByPriceRange(minimumPrice, maximumPrice);
},
);

When('A user click on {string} category, from the returned commercial list', (category) => {
	propertyFinder.getElementByLocatorAndText(propertyFinder.categoryLinks, category).click();
},
);
...
Then('Check the total number of results from the API response matches the total displayed property results.',	() => {
	propertyFinder.getSavedDataFromIntercept('searchResults', '.pageProps.searchResult').then((data) => {
		propertyFinder.getElementByLocator(propertyFinder.totalSearchResultCount)
			.should(	'have.text', propertyFinder.totalSearchResultTexts(data.meta.total_count));

		propertyFinder.getElementByLocator(propertyFinder.propertiesListCards)
			.should('have.length', data.properties.length);
	});
},
);
...
And('Verify the details of the first property in the searched result from the API response', () => {
	propertyFinder.getSavedDataFromIntercept('searchResults', '.pageProps.searchResult.properties.0').then((firstCardData) => {
		...
		propertyFinder.getSavedElement('firstPropertyCard')
			.find(propertyFinder.propertyCardLocation)
			.should('have.text', firstCardData.location.full_name);

		propertyFinder.getSavedElement('firstPropertyCard')
			.find(propertyFinder.propertyCardPrice).invoke('text')
			.should('include', propertyFinder.propertyPriceText(firstCardData.price.value, firstCardData.price.currency));

		propertyFinder.getSavedElement('firstPropertyCard')
			.find(propertyFinder.propertyCardSize)
			.should('have.text',	propertyFinder.propertySizeText(firstCardData.size.value, firstCardData.size.unit));
